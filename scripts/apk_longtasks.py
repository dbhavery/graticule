...
def main() -> None:
    ...
    bad = []
    if worst > MAX_SINGLE_TASK_MS:
        bad.append(f"worst single task {worst} ms > {MAX_SINGLE_TASK_MS} ms")
    if blocking > MAX_TOTAL_BLOCKING_MS:
        bad.append(f"total blocking {blocking} ms > {MAX_TOTAL_BLOCKING_MS} ms")
    print()
    for b in bad:
        print("  FAIL  " + b)
    if not bad:
